<?php
    $url = file_get_contents('https://api.jikan.moe/v3/manga/1/userupdates/1');
...
<body>
    ...
    <div class="row" id="manga-list">
        <div class="row justify-content-center">
            <?php foreach ($menu['users'] as $data): ?>
            <div class="card">
                <div class="col-md-12">
                <center>
                    <img src="<?=$data['image_url']?>" class="rounded" width="800" height="350" alt="">
                    <div class="card-body">
                        <h5 class="card-title"> Username : <?=$data['username']?></h5>
                        <p class="card-text"> Status : <?=$data['status']?></p>
                        <p class="card-text"> Volume Sekarang : <?=$data['volumes_read']?></p>
                        <p class="card-text"> Volume Total : <?=$data['volumes_total']?></p>
                        <p class="card-text"> Chapter Sekarang : <?=$data['chapters_read']?></p>
                        <p class="card-text"> Chapter Total : <?=$data['chapters_total']?></p>
                        <p class="card-text"> Tanggal : <?=$data['date']?></p>
                        <p class="card-text"> Skor : <?=$data['score']?></p>
                        <button type="button" class="btn btn-link"><a href="<?=$data['url']?>" target="_blank" class="text-primary stretched-link"><?=$data['url']?></a></button>
                    </div>
                </center>
                </div>
            </div>
